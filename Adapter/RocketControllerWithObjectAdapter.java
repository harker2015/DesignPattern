...
public class RocketControllerWithObjectAdapter extends JFrame{
	...
	public RocketControllerWithObjectAdapter(){
		...
		runButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					JOptionPane.showMessageDialog(RocketControllerWithObjectAdapter.this, "ERROR: Value for burn area must be a valid double");
					invalidField = true;
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(RocketControllerWithObjectAdapter.this, "ERROR: Value for burn rate must be a valid double");
					invalidField = true;
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(RocketControllerWithObjectAdapter.this, "ERROR: Value for fuel mass must be a valid double");
					invalidField = true;
				}
				...
				try {
					...
					JOptionPane.showMessageDialog(RocketControllerWithObjectAdapter.this, "ERROR: Value for total mass must be a valid double");
					invalidField = true;
				}
				
				// The fuel mass cannot be greater than the total mass of the rocket
				if (fuelMass >= totalMass) {
					JOptionPane.showMessageDialog(RocketControllerWithObjectAdapter.this, "ERROR: Fuel mass must be less than total mass");
					invalidField = true;					
				}
				...
				if (!invalidField) {
					if (!useAdvanced.isSelected()) {
						...
						ObjectMissileAdapter oma = new ObjectMissileAdapter(new RealisticBehaviour(burnArea, burnRate, fuelMass, totalMass));
						// Here you can insert the code for using the advanced simulator with an adapter.
						// Calculating burn time, assuming fuel density is 1800kg/m2
						double burnTime = oma.getBurnTime();
						...
						while (counter < burnTime) {
							// Display mass and thrust each second
							resultsArea.append("At time " + counter + "seconds:\n\n");
							resultsArea.append("Mass: " + oma.getMassInKg() + "kg\n");
							resultsArea.append("Thrust: " + oma.getThrustInNewtonsSec() + "newtons/sec\n\n");
							counter += 1.0;
							// Update simulator's clock
							oma.setSimulatedTime(counter);
						}
						
						// Closing values when rocket has crashed
						oma.setSimulatedTime(burnTime);
						resultsArea.append("At time " + burnTime + "seconds:\n\n");
						resultsArea.append("Mass: " + oma.getMassInKg() + "kg\n");
						resultsArea.append("Thrust: " + oma.getThrustInNewtonsSec() + "newtons/sec\n");
					}
	...
	public static void main(String[] args) {
		RocketControllerWithObjectAdapter c = new RocketControllerWithObjectAdapter();
		c.setSize(470, 550);
		c.setVisible(true);
